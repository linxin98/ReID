...
# 4.1 ResNet-50
# file="${path}/`date +%H%M%S`_uda_resnet-50.log"
# ${python} ${script}/uda_resnet50.py -c config/uda.ini -gpu 0 > ${file} 2>&1 &

# 4.2 Bag of Tricks
# file="${path}/`date +%H%M%S`_uda_bag.log"
# ${python} ${script}/uda_bag.py -c config/uda.ini -gpu 0 > ${file} 2>&1 &

# 5 Test

# 5.1 GMM
# file="${path}/`date +%H%M%S`_gmm.log"
# ${python} ${script}/gmm.py -c config/default.ini -gpu 3 > ${file} 2>&1 &