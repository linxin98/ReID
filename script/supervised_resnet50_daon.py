...
from model import resnet50, classifier, diff_attention
from optimizer import lambda_calculator

if __name__ == '__main__':
    # 0 introduction
    print('Person Re-Identification')
    print('supervised ResNet-50 DAOn')
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save:
            if epoch % save_per_epochs == 0:
                ...
                base_save_name = '[supervised resnet-50 daon]' + time.strftime(
                    "%H%M%S", time.localtime()) + '[base]' + str(epoch) + '.pth'
                torch.save(base_model.state_dict(),
                           os.path.join(save_path, base_save_name))
                diff_save_name = '[supervised resnet-50 daon]' + time.strftime(
